...
- Prefer linking to an existing activity over creating a new one. How similar movements were reconciled in the past is the strongest signal: follow it unless it contradicts the current movement.
- Never create an activity whose name already exists (case-insensitive); link to it instead.
...
const FindSimilarMovementsArgs = z.object({ name: z.string() });

const SearchActivitiesArgs = z.object({
  name: z.string().optional(),
  fromDate: z.string().optional(),
  toDate: z.string().optional(),
});

const LinkMovementArgs = z.object({ activityId: z.string(), amount: z.number() });
...
const GiveUpArgs = z.object({ reason: z.string().optional() });
...
const transcriptToLlmMessages = (messages: WorkflowMessage[]): LlmMessage[] =>
  ...
  const llmMessages: LlmMessage[] = [
    { role: "system", content: SYSTEM_PROMPT },
    {
      ...
      content: JSON.stringify({
        ...
        remainingToAllocate: remainingAmount(state.movement.amount, state.linkAmounts),
        history: {
