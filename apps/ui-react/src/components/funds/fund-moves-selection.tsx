...
import { FundMovesCommandPalette } from "./fund-moves-command-palette";
...
export function FundMovesSelection({
  ...
  const show = selectedFundMoves.length > 0;

  useHotkey("Mod+K", (event) => {
    if (!selectedFundMoves.length) return null;
      ...
      {show && (
        ...
        >
          <div className="flex items-center gap-2 rounded bg-muted p-2 shadow-xl">
            ...
            >
              <span>{selectedFundMoves.length} selected</span>
              <X className="h-4 w-4" />
            </Button>
