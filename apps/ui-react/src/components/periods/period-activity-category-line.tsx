...
export function PeriodActivityCategoryLine({ periodDate, category }: PeriodActivityCategoryLineProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        className={`flex items-center justify-between h-9 rounded px-3 group transition-colors cursor-pointer ${
          viewFilters.category === category.id
            ? 'bg-primary-800'
      ...
      {expanded && (
        <div className="mb-2 space-y-1">
          {categorySubcategories.map((subcategory) => (
            ...
            >
              <div className="text-xs text-primary-700 font-medium flex items-center">
                {subcategory.name}
              </div>

              <div className="flex items-center">
                <div
                  className={`mr-4 text-primary-300 text-sm ${
                    viewFilters.subcategory === subcategory.id
                      ? ''
                      : 'hidden group-hover:block'
                  }`}
                ...
                <div className="whitespace-nowrap text-primary-300 text-sm">
